  ...
  SUBROUTINE initialise_hybrid
    ...
    IF (use_hybrid) THEN
      ...
      DO ix = 1-ng, nx+ng
        ...
        DO i_sol = 1, solid_count
          IF (solid_array(i_sol)%el_density(ix) > max_ne) THEN
            ...
            resistivity_model(ix) = solid_array(i_sol)%res_model
          END IF
        END DO

        ! Do we need ionisation and Coulomb logarithm scripts?
        ! If using Lee-More resistivity
        IF (resistivity_model(ix) == c_resist_rlm) THEN
          use_hy_cou_log = .TRUE.
      ...
      IF (use_hy_ionisation) THEN
        ...
        ALLOCATE(ion_z_avg(1-ng:nx+ng))
        ALLOCATE(ion_reduced_density(1-ng:nx+ng))
        ion_z_avg = 0.0_num
        ion_reduced_density = 0.0_num
      END IF
      IF (use_hy_cou_log) THEN
        ALLOCATE(ion_cou_log(1-ng:nx+ng))
      END IF
      IF (use_ion_temp) THEN
        ALLOCATE(ion_A(1-ng:nx+ng))
        ion_A = 0.0_num
      END IF
      IF (use_ion_temp .OR. use_hy_ionisation) THEN
        ALLOCATE(ion_ni(1-ng:nx+ng))
        ion_ni = 0.0_num
      END IF
