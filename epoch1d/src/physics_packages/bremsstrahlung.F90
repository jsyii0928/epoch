...
CONTAINS
  ...
  SUBROUTINE bremsstrahlung_update_optical_depth
    ...
    INTEGER :: ispecies, iz, z_temp, i, part_type
    TYPE(particle), POINTER :: current
          ...
          DO WHILE(ASSOCIATED(current))
            ...
            IF (use_plasma_screening) THEN
              ...
              CALL grid_centred_var_at_particle(part_x, part_root_te_over_ne, &
                  grid_root_temp_over_num)
              ! Average atomic number at particle position
              CALL grid_centred_var_at_particle(part_x, part_z_avg, ion_z_avg)
              ! Average ion charge state at particle position
              CALL grid_centred_var_at_particle(part_x, part_zstar_avg, &
                  ion_charge)
