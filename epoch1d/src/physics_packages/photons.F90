...
#include "triangle/b_part.inc"
  ...
  FUNCTION epa_cross_sec(v)

    REAL(num), INTENT(IN) :: v
    REAL(num) :: epa_cross_sec

    ! input v is dimensionless lepton velocity in c.o.m.

    epa_cross_sec = quarter_pire2 * (1.0_num - v**2) / v * &
      ( (3.0_num-v**4) / v * LOG((1.0_num+v)/(1.0_num-v)) + &
      2.0_num * (v**2-2.0_num) )

  END FUNCTION epa_cross_sec
#endif
END MODULE photons
