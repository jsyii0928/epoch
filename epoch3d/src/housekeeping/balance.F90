...
CONTAINS
  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    ALLOCATE(temp_int(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
    ...
    IF (use_hybrid) THEN
      ...
      ALLOCATE(resistivity(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
      resistivity = temp

      CALL remap_field_int(resistivity_model, temp_int)
      DEALLOCATE(resistivity_model)
      ALLOCATE(resistivity_model(1-ng:nx_new+ng, 1-ng:ny_new+ng, &
          1-ng:nz_new+ng))
      ...
      IF (use_hy_ionisation) THEN
        ...
        ALLOCATE(ion_charge(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
        ion_charge = temp

        CALL remap_field(ion_z_avg, temp)
        DEALLOCATE(ion_z_avg)
        ALLOCATE(ion_z_avg(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
        ion_z_avg = temp

        CALL remap_field(ion_reduced_density, temp)
        DEALLOCATE(ion_reduced_density)
        ALLOCATE(ion_reduced_density(1-ng:nx_new+ng, 1-ng:ny_new+ng, &
            1-ng:nz_new+ng))
        ion_reduced_density = temp
      END IF

      IF (use_hy_cou_log) THEN
        CALL remap_field(ion_cou_log, temp)
        DEALLOCATE(ion_cou_log)
        ALLOCATE(ion_cou_log(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
        ion_cou_log = temp
      END IF
      ...
      IF (use_ion_temp) THEN
        ...
        DEALLOCATE(ion_a)
        ALLOCATE(ion_a(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
        ion_a = temp

        CALL remap_field(hy_ti, temp)
        DEALLOCATE(hy_ti)
        ALLOCATE(hy_ti(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
        hy_ti = temp
      END IF
      ...
      IF (use_ion_temp .OR. use_hy_ionisation) THEN
        ...
        DEALLOCATE(ion_ni)
        ALLOCATE(ion_ni(1-ng:nx_new+ng, 1-ng:ny_new+ng, 1-ng:nz_new+ng))
        ion_ni = temp
      END IF
    ...
    DO WHILE(ASSOCIATED(hy_laser_next))
      ...
      ALLOCATE(hy_laser%profile(1-ng:ny_new+ng, 1-ng:nz_new+ng))
    ...
    DO WHILE(ASSOCIATED(hy_laser_next))
      ...
      DEALLOCATE(hy_laser%profile)
      ALLOCATE(hy_laser%profile(1-ng:nx_new+ng, 1-ng:nz_new+ng))
      hy_laser%profile = temp_slice
    END DO
    ...
    DO WHILE(ASSOCIATED(hy_laser_next))
      ...
      IF (hy_laser%boundary == c_bd_z_min) THEN
        max_boundary = .FALSE.
      ELSE IF (hy_laser%boundary == c_bd_z_max) THEN
        max_boundary = .TRUE.
      ELSE
        CYCLE
      END IF

      CALL remap_field_slice(c_dir_z, hy_laser%profile, temp_slice)
      DEALLOCATE(hy_laser%profile)
      ALLOCATE(hy_laser%profile(1-ng:nx_new+ng, 1-ng:ny_new+ng))
      hy_laser%profile = temp_slice
    END DO
  ...
  SUBROUTINE remap_field_int(field_in, field_out)

    ! This is a wrapper for the field redistribution routine
    INTEGER, DIMENSION(:,:,:), INTENT(IN) :: field_in
    INTEGER, DIMENSION(:,:,:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(c_ndims) :: n_new, cdim
    INTEGER :: i
    ...
    CALL redistribute_field_3d_int(field_in, field_out, cdim, &
        cell_x_min, cell_x_max, new_cell_x_min, new_cell_x_max, &
        cell_y_min, cell_y_max, new_cell_y_min, new_cell_y_max, &
        cell_z_min, cell_z_max, new_cell_z_min, new_cell_z_max)

    CALL do_field_mpi_with_lengths_int(field_out, ng, n_new(1), n_new(2), &
        n_new(3))

  END SUBROUTINE remap_field_int
  ...
  SUBROUTINE redistribute_field_3d_int(field_in, field_out, cdim, &
      old_cell_min1, old_cell_max1, new_cell_min1, new_cell_max1, &
      old_cell_min2, old_cell_max2, new_cell_min2, new_cell_max2, &
      old_cell_min3, old_cell_max3, new_cell_min3, new_cell_max3)
    ...
    ! This subroutine redistributes the fields over the new processor layout
    INTEGER, PARAMETER :: nd = 3
    INTEGER, DIMENSION(1-ng:,1-ng:,1-ng:), INTENT(IN) :: field_in
    INTEGER, DIMENSION(1-ng:,1-ng:,1-ng:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(nd), INTENT(IN) :: cdim
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min1, old_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: new_cell_min1, new_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min2, old_cell_max2
  ...
  END SUBROUTINE redistribute_field_3d_int
