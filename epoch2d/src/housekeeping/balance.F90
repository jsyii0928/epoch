...
CONTAINS
  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    ALLOCATE(temp_int(1-ng:nx_new+ng, 1-ng:ny_new+ng))
    ...
    IF (use_hybrid) THEN
      ...
      ALLOCATE(resistivity(1-ng:nx_new+ng, 1-ng:ny_new+ng))
      resistivity = temp
      ...
      DEALLOCATE(hy_te)
      ALLOCATE(hy_te(1-ng:nx_new+ng, 1-ng:ny_new+ng))
      hy_te = temp

      CALL remap_field(jbx, temp)
      DEALLOCATE(jbx)
      ALLOCATE(jbx(1-ng:nx_new+ng, 1-ng:ny_new+ng))
      jbx = temp

      CALL remap_field(jby, temp)
      DEALLOCATE(jby)
      ALLOCATE(jby(1-ng:nx_new+ng, 1-ng:ny_new+ng))
      jby = temp

      CALL remap_field(jbz, temp)
      DEALLOCATE(jbz)
      ALLOCATE(jbz(1-ng:nx_new+ng, 1-ng:ny_new+ng))
      jbz = temp

      IF (use_hy_ionisation) THEN
        CALL remap_field(ion_charge, temp)
        DEALLOCATE(ion_charge)
        ALLOCATE(ion_charge(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        ion_charge = temp

        CALL remap_field(ion_z_avg, temp)
        DEALLOCATE(ion_z_avg)
        ALLOCATE(ion_z_avg(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        ion_z_avg = temp
      ...
      IF (use_hy_cou_log) THEN
        ...
        ALLOCATE(ion_cou_log(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        ion_cou_log = temp
      END IF
      ...
      IF (use_ion_temp .OR. use_hy_ionisation) THEN
        ...
        DEALLOCATE(ion_ni)
        ALLOCATE(ion_ni(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        ion_ni = temp
      END IF
  ...
  SUBROUTINE remap_field_int(field_in, field_out)
    ...
    CALL redistribute_field_2d_int(field_in, field_out, cdim, &
        cell_x_min, cell_x_max, new_cell_x_min, new_cell_x_max, &
        cell_y_min, cell_y_max, new_cell_y_min, new_cell_y_max)
  ...
  SUBROUTINE redistribute_field_2d_int(field_in, field_out, cdim, &
      old_cell_min1, old_cell_max1, new_cell_min1, new_cell_max1, &
      old_cell_min2, old_cell_max2, new_cell_min2, new_cell_max2)

    ! This subroutine redistributes the fields over the new processor layout
    INTEGER, PARAMETER :: nd = 2
    INTEGER, DIMENSION(1-ng:,1-ng:), INTENT(IN) :: field_in
    INTEGER, DIMENSION(1-ng:,1-ng:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(nd), INTENT(IN) :: cdim
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min1, old_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: new_cell_min1, new_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min2, old_cell_max2
  ...
  END SUBROUTINE redistribute_field_2d_int
