...
CONTAINS
  ...
  SUBROUTINE get_heat_capacity
    ...
    IF (use_hybrid_collisions) THEN
      ALLOCATE(ion_heat_const(1-ng:nx+ng,1-ng:ny+ng))
      ion_heat_const = i_kb_sumne2 * idx * idy
    END IF
  ...
  SUBROUTINE ionisation_heating(de, part_x, part_y)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_heat_const, &
        ion_heat_const)
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_c_eff, &
        eff_heat_capacity)
...
#endif
    hy_te(ix,iy) = hy_te(ix,iy) + delta_te
