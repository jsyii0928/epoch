...
CONTAINS
  ...
  SUBROUTINE run_single_laser(laser)
    ...
    INTEGER :: dir_index, icell, idir, ipart
    REAL(num) :: area, vol
    ...
    IF (boundary == c_bd_x_min) THEN
      ...
      n_perp_cell = (/ny/)                  ! Number of cells along boundary
      perp_cell_size = (/dy/)               ! Perpendicular dimensions of cell
      perp_dir_index = (/2/)                ! Perpendicular directions
    ...
    DO icell = 1, n_perp_cell(1)
      ...
      cur_cell = 0.0_num
      DO idir = 1, c_ndims-1
        IF (perp_dir_index(idir) == 1) cur_cell(idir) = x(icell)
        IF (perp_dir_index(idir) == 2) cur_cell(idir) = y(icell)
      END DO
