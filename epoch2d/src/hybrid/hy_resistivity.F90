...
CONTAINS
  ...
  SUBROUTINE update_ionisation
    ...
    DO iy = 1-ng, ny+ng
      DO ix = 1-ng, nx+ng
        ion_charge(ix,iy) = thomas_fermi_ionisation(ix,iy)
      END DO
    END DO
  ...
  SUBROUTINE update_coulomb_logarithm
    ...
    DO iy = 1-ng, ny+ng
      DO ix = 1-ng, nx+ng
        ion_cou_log(ix,iy) = lee_more_coulomb_log(ix,iy)
      END DO
    END DO
    ...
    DO iy = 1-ng, ny+ng
      DO ix = 1-ng, nx+ng
        ...
        CASE(c_resist_milchberg)
          resistivity(ix,iy) = calc_resistivity_milchberg(hy_te(ix,iy))
        CASE(c_resist_plastic)
          resistivity(ix,iy) = calc_resistivity_plastic(hy_te(ix,iy))
        CASE(c_resist_rlm)
          resistivity(ix,iy) = calc_resistivity_rlm(ix,iy)
        END SELECT
      END DO
    END DO
  ...
  FUNCTION calc_resistivity_rlm(ix, iy)
    ...
    rlm_z_star = ion_charge(ix,iy)
    rlm_cou_log = ion_cou_log(ix,iy)
    rlm_te = hy_te(ix,iy)
    rlm_ni = ion_ni(ix,iy)
  ...
  FUNCTION thomas_fermi_ionisation(ix, iy)
    ...
    tf_t0 = hy_te(ix,iy) * kelvin_to_ev * &
        ion_z_avg(ix,iy)**(-4.0_num/3.0_num)
    ...
    tf_q1 = tf_a * ion_reduced_density(ix,iy)**tf_b
    tf_q = (ion_reduced_density(ix,iy)**tf_c + tf_q1**tf_c)**(1.0_num/tf_c)

    tf_x = 14.3139_num * tf_q**0.6624_num
    thomas_fermi_ionisation = ion_z_avg(ix,iy)*tf_x &
  ...
  FUNCTION lee_more_coulomb_log(ix, iy)
